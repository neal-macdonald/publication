...
@keras_export('keras.initializers.NovelMethod',
              'keras.initializers.novel_method',
              v1=[])
class NovelMethod(tf.keras.initializers.VarianceScaling):
    
    def __init__(self, kernel, dilation, scale=1., sparse_out=False, seed=None):
        super(NovelMethod, self).__init__(
            scale=scale, 
            mode='fan_avg',
            distribution='truncated_normal', 
            seed=seed)
    ...
    def __call__(self, shape, dtype='float32', **kwargs):
        scale = self.scale
        sparse_out = self.sparse_out
        stddev = 0.05 # math.sqrt(scale) / .87962566103423978
        dilation = self.dilation
        kernel = self.kernel
        nonzero_count = self.kernel**2
        ...
        def get_novel_sparse_tensor():
            values = self._random_generator.truncated_normal(shape=[shape[0],shape[1]],
                                             mean=0.0,stddev=stddev,dtype=dtype).numpy().flatten()
            nonzero_indices = set()
            ...
            if sparse_out == True:
                index2d = np.unravel_index(list(nonzero_indices),[dilated_height,dilated_width])
                index_formatted = [[x,y] for x,y in zip(index2d[0],index2d[1])]
                tensor = tf.SparseTensor(index_formatted,values,(dilated_height,dilated_width))
            else:
    ...
    def get_config(self):
        return {'dilation': self.dilation,
                'seed': self.seed}

@keras_export('keras.initializers.GlorotNormInit',
              'keras.initializers.glorot_norm_init',
              v1=[])
class GlorotNormInit(tf.keras.initializers.VarianceScaling):
    
    def __init__(self, kernel, dilation, scale=1.0, seed=None):
 
        super(GlorotNormInit, self).__init__(
            scale=scale,
            mode='fan_avg',
            distribution='truncated_normal',
            seed=seed)
    ...
    def __call__(self, dtype='float32', **kwargs): # Removed shape argument from function
    
        stddev = math.sqrt(self.scale) / .87962566103423978
        values = self._random_generator.truncated_normal(shape=(self.kernel, self.kernel),
                                                         mean=0.0,
                                                         stddev=stddev,
                                                         dtype=dtype).numpy()

        if self.dilation == 1:
            return np.expand_dims(np.expand_dims(values,2),3)
        elif self.dilation > 1:
            laterals = [x*self.dilation for x in range(self.kernel)]
            verticals = [y*self.dilation for y in range(self.kernel)]
            edges = []
            for x in laterals:
                for y in verticals:
                    edges.append([x,y])
            ...
            for x in range(self.kernel):
                for y in range(self.kernel):
                    ziplist.append([x,y])
                arr = np.zeros((self.kernel-(self.dilation-1)+(self.kernel*(self.dilation-1)),
                               self.kernel-(self.dilation-1)+(self.kernel*(self.dilation-1))))
                for edge,index in zip(edges,ziplist):
                    x,y = edge
                    m,n = index
                    arr[x,y] = values[m,n]
            return np.expand_dims(np.expand_dims(arr,2),3)
    
@keras_export('keras.initializers.RandomUniformInit',
              'keras.initializers.random_uniform_init',
              v1=[])
class RandomUniformInit(tf.keras.initializers.RandomUniform):
    
    def __init__(self, kernel, dilation, minval=-1, maxval=1, seed=None):
        if seed == None:
            self.seed = int(tf.random.uniform([1],0,999999).numpy()[0])
        else:
            self.seed = seed
        self.kernel = kernel
        self.dilation = dilation
    ...
    def __call__(self, dtype='float32', **kwargs):
        values = self._random_generator.uniform(shape=(self.kernel, self.kernel),
                                                minval=self.minval,
                                                maxval=self.maxval,
